...
**MinIO photo URLs needed a second disk.** Evidence photos are stored via the app container's Docker-internal view of MinIO (`AWS_ENDPOINT=http://minio:9000`), but a presigned URL opened in the admin's own browser can't resolve that hostname. Added `s3_public` in `config/filesystems.php` — same bucket/credentials, but `AWS_ENDPOINT_PUBLIC` (defaults to `http://localhost:9000`, MinIO's published port) instead. In staging/production this collapses to a no-op (`AWS_ENDPOINT_PUBLIC` = `AWS_ENDPOINT`, both the real public S3/R2 endpoint).

**Evidence photo URLs are disk-agnostic (added 2026-09-23, first real VPS deploy).** `ActivityMonitoringController` now calls `Storage::temporaryUrl(...)` on the *default* disk rather than hardcoding `s3_public` — because a real deployment may have no S3/R2 account yet and use `FILESYSTEM_DISK=local` instead (see `DEPLOYMENT.md`). Laravel's local disk has no `temporaryUrl()` support out of the box, so `AppServiceProvider` registers `buildTemporaryUrlsUsing()` for it, backed by a signed, time-limited route (`evidence-photos.show` in `routes/web.php`) — same "possess the link, nothing more" trust model S3's presigned URLs already had, not a new/weaker check. Covered by `tests/Feature/Evidence/LocalEvidencePhotoUrlTest.php` and a real headless-browser render check (not just green tests — this is exactly the kind of thing that looks fine in `php artisan test` and renders a broken image icon in a real browser). That browser check *did* catch a real bug: a locally-stored photo created with the wrong file owner 404s with no other symptom — `DEPLOYMENT.md`'s `chown -R www-data:www-data storage` step is load-bearing here, not cosmetic. Also found in the same session: **MinIO's standalone server/client binaries are discontinued upstream** (`dl.min.io` and GitHub Releases both now 404/410) — breaks installing MinIO on a plain server (not Docker, where the image still pulls fine); `FILESYSTEM_DISK=local` exists partly because of this.
